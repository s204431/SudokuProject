...
public class SolverTester {
  ...
  public void test(Model model, SudokuSolver solver) {
    ...
    SolverTestCase[] testCases =
        new SolverTestCase[] {
          new SolverTestCase("given", true),
      ...
      if (solver.recursiveCalls >= 500000) {
        System.out.println(
            solver.getClass().getSimpleName()
                + " took too long for test \""
                + testCase.fileName
                + "\".");
        success = false;
      } else if (testCase.solvable
          && solutions.size() > 0
          && Model.sudokuSolved(solutions.get(0), model.innerSquareSize)) {
        System.out.println(
            solver.getClass().getSimpleName()
                + " passed test \""
                + testCase.fileName
        ...
        System.out.println(
            solver.getClass().getSimpleName() + " failed test \"" + testCase.fileName + "\".");
        success = false;
      }
    }
      ...
      if (successGenerated) {
        System.out.println(
            solver.getClass().getSimpleName()
                ...
                + " recursive calls and "
                + guesses
                + " guesses. Average difficulty: "
                + sumOfDifficulties / (double) numberOfRandomTests
                + ".");
      } else {
        success = false;
      }
    ...
    if (success) {
      System.out.println(
          solver.getClass().getSimpleName()
              + " passed all tests in "
              + (new Date().getTime() - start - timeToGenerate)
              + " ms.");
    } else {
