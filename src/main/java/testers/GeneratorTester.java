...
public class GeneratorTester {
  public void test(Model model) {
    ...
    GeneratorTestCase[] testCases =
        new GeneratorTestCase[] {
          new GeneratorTestCase(50, "Easy", 3, 3),
        ...
        if (testCase.difficulty.equals("")) {
          result =
              generator.generateSudoku(
                  testCase.innerSquareSize,
                  testCase.numInnerSquares,
                  testCase.minDifficulty,
                  testCase.maxDifficulty,
                  0);
        } else {
          int[] range = SudokuSolver.getDifficultyRange(testCase.difficulty);
          result =
              generator.generateSudoku(
                  testCase.innerSquareSize, testCase.numInnerSquares, range[0], range[1], 0);
        }
        SudokuSolver solver = new EfficientSolver(result, testCase.innerSquareSize);
        int[] range =
            testCase.difficulty.equals("")
                ? new int[] {testCase.minDifficulty, testCase.maxDifficulty}
                : SudokuSolver.getDifficultyRange(testCase.difficulty);
        if (solver.solve(2).size() != 1
            || !(solver.difficulty >= range[0] && solver.difficulty <= range[1])) {
  ...
  private class GeneratorTestCase {
    ...
    public GeneratorTestCase(
        int amount, String difficulty, int innerSquareSize, int numInnerSquares) {
      this.amount = amount;
