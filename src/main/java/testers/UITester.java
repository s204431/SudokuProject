...
public class UITester extends ComponentTestFixture {
      ...
      for (int i = 0; i < 200; i++) {
        int randomX =
            ThreadLocalRandom.current()
                .nextInt(centerX - Main.SCREEN_WIDTH / 3, centerX + Main.SCREEN_WIDTH / 3);
        int randomY =
            ThreadLocalRandom.current()
                .nextInt(centerY - Main.SCREEN_HEIGHT / 3, centerY + Main.SCREEN_HEIGHT / 3);
  ...
  private void checkDifficulty(
      String difficulty,
      String initialButtonName,
      boolean allowRemovingInitialValues,
    ...
    assertEquals(
        "Sudoku has wrong difficulty.",
        difficulty,
        SudokuSolver.getDifficultyString(solver.difficulty));
  ...
  private void checkSudokuActions(View view, Model model, boolean allowRemovingInitialValues) {
    ...
    tester.click(
        view,
        view.boardX + y * view.fieldWidth + view.fieldWidth / 2,
        view.boardY + x * view.fieldHeight + view.fieldHeight / 2);
    ...
    assertTrue(
        ...
        view.fieldWidth == initialFieldWidth
            && view.fieldHeight == initialFieldHeight
            && view.boardX == initialBoardX
            && view.boardY == initialBoardY);
  }
  ...
  private void checkDifficultyDropdown(String[] expectedChoices) {
    ...
    assertEquals(
        "Wrong choices in difficulty dropdown.", expectedChoices.length, dropdown.getItemCount());
    for (int i = 0; i < expectedChoices.length; i++) {
      assertTrue(
          "Wrong choices in difficulty dropdown.",
          expectedChoices[i].equals(dropdown.getItemAt(i)));
    }
  }

  private void performButtonClickSequence(String[] buttonTexts, Class<?>[] expectedPanelClasses) {
    assertTrue(
        "Buttons and expected panel classes must have same lengts.",
        buttonTexts.length == expectedPanelClasses.length);
