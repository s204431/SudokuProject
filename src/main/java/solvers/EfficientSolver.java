...
public class EfficientSolver extends SudokuSolver {
    ...
    for (int i = 0; i < board.length; i++) {
      for (int j = 0; j < board.length; j++) {
        for (int v : possibleValues[i][j]) {
          ...
          for (int k = i / innerSquareSize * innerSquareSize;
              k < i / innerSquareSize * innerSquareSize + innerSquareSize;
              k++) {
            for (int l = j / innerSquareSize * innerSquareSize;
                l < j / innerSquareSize * innerSquareSize + innerSquareSize;
                l++) {
            ...
            if (foundInnerSquares.size() == foundRows.size() - 1) {
              for (int k = 0; k < getNumInnerSquares(); k++) {
                if (k != j && !foundInnerSquares.contains(k)) {
                  for (int r = i * innerSquareSize;
                      r < i * innerSquareSize + innerSquareSize;
                      r++) {
                    for (int c = k * innerSquareSize;
                        c < k * innerSquareSize + innerSquareSize;
                        c++) {
            ...
            if (foundInnerSquares.size() == foundColumns.size() - 1) {
              for (int k = 0; k < getNumInnerSquares(); k++) {
                if (k != i && !foundInnerSquares.contains(k)) {
                  for (int r = k * innerSquareSize;
                      r < k * innerSquareSize + innerSquareSize;
                      r++) {
                    for (int c = j * innerSquareSize;
                        c < j * innerSquareSize + innerSquareSize;
                        c++) {
    ...
    for (int i = 0; i < board.length; i++) {
      for (int j = 0; j < board[i].length; j++) {
        ...
        if (foundPositions.size() > 0
            && foundPositions.size() < board.length
            && distinctValues.size() == foundPositions.size()) {
          for (int r = 0; r < board.length; r++) {
            ...
            if (cs[0] == cs2[0] && cs[1] == cs2[1]) {
              for (int r = 0; r < board.length; r++) {
                if (r != i && r != k) {
                  ...
                  if (size1 != possibleValues[r][cs[0]].size()
                      || size2 != possibleValues[r][cs[1]].size()) {
                    updated = true;
                  }
                }
              }
            ...
            if (rs[0] == rs2[0] && rs[1] == rs2[1]) {
              for (int c = 0; c < board.length; c++) {
                if (c != i && c != k) {
                  ...
                  if (size1 != possibleValues[rs[0]][c].size()
                      || size2 != possibleValues[rs[1]][c].size()) {
                    updated = true;
                  }
                }
              }
                  ...
                  if (cs3[0] == cs[i1] && cs3[1] == cs2[i2]) {
                    for (int r = 0; r < board.length; r++) {
                      if (r != i && r != k && r != l) {
                        ...
                        if (size1 != possibleValues[r][cs3[0]].size()
                            || size2 != possibleValues[r][cs3[1]].size()
                            || size3 != possibleValues[r][cs[i1 == 0 ? 1 : 0]].size()) {
                          updated = true;
                        }
                      }
                    }
                  ...
                  if (rs3[0] == rs[i1] && rs3[1] == rs2[i2]) {
                    for (int c = 0; c < board.length; c++) {
                      if (c != i && c != k && c != l) {
                        ...
                        if (size1 != possibleValues[rs3[0]][c].size()
                            || size2 != possibleValues[rs3[1]][c].size()
                            || size3 != possibleValues[rs[i1 == 0 ? 1 : 0]][c].size()) {
                          updated = true;
                        }
                      }
                    }
  ...
  // Used for X-Wing and Swordfish to check if a row or column contains a specific possible value in
  // exactly two fields.
  ...
  private void forcingChainsRecursive(
      int[][] board, List<Integer>[][] possibleValues, List<int[]> forces, int x, int y) {
    for (int i = 0; i < board.length; i++) {
    ...
    for (int i = x / innerSquareSize * innerSquareSize;
        i < x / innerSquareSize * innerSquareSize + innerSquareSize;
        i++) {
      for (int j = y / innerSquareSize * innerSquareSize;
          j < y / innerSquareSize * innerSquareSize + innerSquareSize;
          j++) {
        if ((i != x || j != y)) {
  ...
  private boolean detectUnsolvable(int[][] board, List<Integer>[][] possibleValues) {
    ...
    boolean[][] foundInnerSquare =
        new boolean[getNumInnerSquares() * getNumInnerSquares()][getMaxValue()];
      ...
      for (int j = 0; j < board.length; j++) {
        ...
        if (board[i][j] < 1) {
          for (int v : possibleValues[i][j]) {
            ...
            foundInnerSquare[(i / innerSquareSize) * getNumInnerSquares() + (j / innerSquareSize)][
                    v - 1] =
                true;
          }
        } else {
  ...
  private List<Integer>[][] copyOf(List<Integer>[][] possibleValues) {
    List<Integer>[][] newPossibleValues =
        new ArrayList[possibleValues.length][possibleValues[0].length];
  ...
  public int[] makeOneMove() {
    ...
    if (solvedSudoku.size() > 0) {
      ...
      if (solvedSudoku.get(0)[move[0].get(0)][move[0].get(1)] == move[1].get(0)) {
        ...
        return new int[] {
          move[0].get(0), move[0].get(1), solvedSudoku.get(0)[move[0].get(0)][move[0].get(1)]
        };
      }
    }
    return null;
