...
public class View extends JPanel implements MouseListener, KeyListener, MouseWheelListener {
  ...
  public void paint(Graphics g) {
    ...
    for (int i = 0; i < model.getBoardSize(); i++) {
      for (int j = 0; j < model.getBoardSize(); j++) {
        ...
        if (model.board[i][j].value
                == // marks other fields
                model.getBoard()[clickedPosition[0]][clickedPosition[1]]
                    .getValue() // with same value
        ...
        if (value > 0 && value <= model.innerSquareSize * model.innerSquareSize) {
          ...
          g2.setFont(
              new Font("Courier", Font.BOLD, (40 - scaling) * fieldWidth / Field.DEFAULT_WIDTH));
          String text = "" + value;
          ...
          if (!Model.canBePlaced(model.board, model.innerSquareSize, i, j, value)) {
            ...
            g2.drawString(
                text,
                boardX + j * fieldWidth + fieldWidth / 2 - fontWidth / 2,
                boardY + i * fieldHeight + fieldHeight / 2 + fontHeight / 3);
          } else {
            g2.drawString(
                text,
                boardX + j * fieldWidth + fieldWidth / 2 - fontWidth / 2,
          ...
          for (int k = 0; k < 9; k++) {
            ...
            g2.drawString(
                text,
                boardX + j * fieldWidth + fieldWidth / 3 * ((note - 1) % 3) + fontWidth,
                boardY + i * fieldHeight - fontHeight / 3 + fieldHeight / 3 * ((note - 1) / 3 + 1));
          }
        }
      }
    }
    ...
    for (int i = 0; i < model.numInnerSquares; i++) {
      for (int j = 0; j < model.numInnerSquares; j++) {
        // g2.setColor(model.sudokuSolved(model.board, model.innerSquareSize) ? new Color(0, 200, 0)
        // : black);
        g2.setColor(black);
        g2.setStroke(new BasicStroke(3));
        g2.drawRect(
            boardX + j * fieldWidth * iss,
            boardY + i * fieldHeight * iss,
            fieldWidth * iss,
            fieldHeight * iss);
      }
    }
  ...
  public void addComponentsToButtonPanel(Mode mode) {
    ...
    if (mode == Mode.play) {
      ...
      stepSolveButton.addActionListener(
          new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent e) {
              model.giveHint();
  ...
  public void showSavePopup() {
    ...
    JLabel boardSizeLabel =
        new JLabel("Size: " + model.getBoardSize() + "x" + model.getBoardSize());
        ...
        new ActionListener() {
          ...
          public void actionPerformed(ActionEvent e) {
            ...
            difficultyLabel.setText(
                "Difficulty: " + savedDifficulty + " (" + difficultyString + ")");
    ...
    String a =
        (String)
            JOptionPane.showInputDialog(
                frame, panel, "Save", JOptionPane.QUESTION_MESSAGE, null, null, model.fileName);
    if (a != null) {
  ...
  public void mouseClicked(MouseEvent e) {
    ...
    if (e.getButton() == MouseEvent.BUTTON1
        && e.getX() >= boardX
        && e.getY() >= boardY
        && e.getX() <= boardX + model.getBoardSize() * fieldWidth
        && e.getY() <= boardY + model.getBoardSize() * fieldHeight) {
      clickedPosition =
          new int[] {(e.getY() - boardY) / fieldHeight, (e.getX() - boardX) / fieldWidth};
      repaint();
    }
  }
  ...
  public void winPopup(int difficulty) {
    ...
    JOptionPane.showMessageDialog(
        frame, "Sudoku solved! your time was: " + hour + ":" + minute + ":" + second);
