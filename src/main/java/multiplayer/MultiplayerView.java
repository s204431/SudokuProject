...
public class MultiplayerView extends View {
  ...
  public MultiplayerView(MultiplayerModel model) {
    ...
    try {
      ...
      ((MultiplayerModel) model)
          .toOpponent.put("boardposition", boardX, boardY, fieldWidth, fieldHeight);
  ...
  public void addOpponentBoard() {
    ...
    opponentPanel.setBounds(
        (getPreferredSize().width - 200) / 2,
        0,
        (getPreferredSize().width - 200) / 2,
  ...
  public void paint(Graphics g) {
    ...
    if (!((MultiplayerModel) model).started) {
      ...
      g2.drawString(
          ...
          getPreferredSize().width / 2 - getPreferredSize().height / 4,
          getPreferredSize().height / 2 - getPreferredSize().height / 10);
      buttonPanel.repaint();
      return;
    }
    ...
    g2.drawLine(
        (getPreferredSize().width - 200) / 2 - 1,
        0,
        (getPreferredSize().width - 200) / 2 - 1,
        getPreferredSize().height);
  }
  ...
  private void updateBoardPosition() {
    try {
      ((MultiplayerModel) model)
          .toOpponent.get(new ActualField("lock"), new ActualField("dragging"));
      ((MultiplayerModel) model)
          .toOpponent.get(
              new ActualField("boardposition"),
  ...
  private class OpponentBoard extends JPanel {
    ...
    public void paint(Graphics g) {
      ...
      try {
        ((MultiplayerModel) model)
            .fromOpponent.get(new ActualField("lock"), new ActualField("dragging"));
        tuple =
            ((MultiplayerModel) model)
                .fromOpponent.query(
                    new ActualField("boardposition"),
      ...
      int[] opponentBoardPosition =
          convertCoordinate((int) tuple[1], (int) tuple[2], (int) tuple[3]);
      ...
      for (int i = 0; i < model.getBoardSize(); i++) {
        for (int j = 0; j < model.getBoardSize(); j++) {
          g2.setColor(white);
          g2.fillRect(
              opponentBoardX + j * opponentFieldWidth,
              opponentBoardY + i * opponentFieldHeight,
              opponentFieldWidth,
              opponentFieldHeight);
          g2.setColor(black);
          g2.drawRect(
          ...
          g2.setFont(
              new Font(
                  "Courier", Font.BOLD, (40 - scaling) * opponentFieldWidth / Field.DEFAULT_WIDTH));
          String text = "" + value;
          int fontHeight = g2.getFontMetrics().getHeight();
          int fontWidth = g2.getFontMetrics().stringWidth(text);
          if (value > 0 && value <= model.innerSquareSize * model.innerSquareSize) {
            int x =
                opponentBoardX + j * opponentFieldWidth + opponentFieldWidth / 2 - fontWidth / 2;
            int y =
                opponentBoardY + i * opponentFieldHeight + opponentFieldHeight / 2 + fontHeight / 3;
      ...
      for (int i = 0; i < model.numInnerSquares; i++) {
        for (int j = 0; j < model.numInnerSquares; j++) {
          // g2.setColor(model.sudokuSolved(model.board, model.innerSquareSize) ? new Color(0, 200,
          // 0) : black);
          g2.setColor(black);
          g2.setStroke(new BasicStroke(3));
          g2.drawRect(
              opponentBoardX + j * opponentFieldWidth * iss,
              opponentBoardY + i * opponentFieldHeight * iss,
              opponentFieldWidth * iss,
              opponentFieldHeight * iss);
        }
      }
